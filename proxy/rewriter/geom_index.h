...
// The shim's own record layout (phase5/rewriter/shimtrace.py). With
// `copies` 0 every TraceRay traces the structure at t0, space
// kGeomIndexSpace with RayContributionToHitGroupIndex = the index of its
// (R, M) in `pairs` and a multiplier of pairs.size(). With `copies` N >= 1
// (arguments computed at run time, or more than 15 pairs) it reads its pair
